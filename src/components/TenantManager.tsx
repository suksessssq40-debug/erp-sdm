...
export const TenantManager: React.FC<{ store: ReturnType<typeof useStore> }> = ({ store }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        await store.createTenant(formData.id, formData.name, formData.description, selectedFeatures);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        await store.updateTenant(editingId, formData.name, formData.description, selectedFeatures, formData.isActive);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em] mt-2 ml-14 md:ml-16">Pusat Kendali Eksekutif Multi-Unit</p>
        </div>
        {view === 'LIST' && (
            <button 
            ...
            <form onSubmit={view === 'CREATE' ? handleCreate : handleUpdate} className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-8">
                        <div>
                            <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-4 italic">Informasi Utama</label>
                            <input 
                                value={formData.name}
                                onChange={e => {
                                    ...
                                    setFormData(prev => ({
                                        ...prev, 
                                        name: val,
                                        id: view === 'CREATE' ? (prev.id ? prev.id : val.toLowerCase().replace(/[^a-z0-9]/g, '-')) : prev.id
                                    }));
                                }}
                                ...
                                placeholder="Nama Unit Bisnis (Contoh: SDM Digital)"
                            />
                        </div>
                        <div>
                            <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-4 italic">Unit Identifier (URL Slug)</label>
                            <div className="flex items-center">
                                <span className="bg-slate-100 text-slate-400 font-black px-8 py-5 rounded-l-3xl border-y-2 border-l-2 border-slate-100 tracking-widest text-lg">/</span>
                                <input 
                                    value={formData.id}
                                    disabled={view === 'EDIT'}
                                    onChange={e => setFormData({...formData, id: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '')})}
                                    className={`w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-r-3xl font-bold text-slate-700 focus:border-blue-500 focus:bg-white outline-none transition-all shadow-inner text-lg ${view === 'EDIT' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    placeholder="sdm-digital"
                                />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 max-h-[400px] overflow-y-auto pr-3 custom-scrollbar">
                            {moduleList.map(mod => (
                                <button
                                    ...
                                    className={`px-8 py-5 rounded-3xl border-2 font-black text-xs transition-all flex items-center justify-between group ${
                                        selectedFeatures.includes(mod.id) 
