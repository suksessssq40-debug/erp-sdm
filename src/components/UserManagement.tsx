...
import { Plus, Bot, Layout as LayoutIcon, Unlock, Download, Upload, Pencil, Trash2, X, Save, Target } from 'lucide-react';
...
export const UserManagement: React.FC<UserManagementProps> = ({ users, currentUser, onAddUser, onUpdateUser, onDeleteUser, onResetDevice, toast, store }) => {
  ...
  const [formData, setFormData] = useState({ name: '', username: '', telegramId: '', telegramUsername: '', role: 'STAFF', password: '', passwordConfirm: '', isFreelance: false, isKaizenMaster: false, tenantId: currentUser?.tenantId || 'sdm' });
  ...
  const resetForm = () => {
    setFormData({ name: '', username: '', telegramId: '', telegramUsername: '', role: 'STAFF', password: '', passwordConfirm: '', isFreelance: false, isKaizenMaster: false, tenantId: currentUser?.tenantId || 'sdm' });
    setTargetUser(null);
  };
  ...
  const handleOpenEdit = (user: User) => {
    ...
    setFormData({
      ...
      isKaizenMaster: user.isKaizenMaster || false,
    ...
    try {
      if (isEdit && targetUser) {
        const updatedUser: User = {
          ...
          isKaizenMaster: formData.isKaizenMaster,
              ...
              {filteredUsers.map((u: any) => (
                <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    ...
                    {u.isKaizenMaster && (
                       <span className="ml-2 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-purple-50 text-purple-600 border border-purple-200">
                          KAIZEN ({(u as any).kaizenPoints ?? 100})
                       </span>
                    )}
                    {u.isFreelance && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-2xl space-y-8 shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              {/* KAIZEN MASTER TOGGLE (OWNER only) */}
              {currentUser?.role === UserRole.OWNER && (
              <div className="bg-purple-50 p-6 rounded-[2rem] border border-purple-100 flex items-center justify-between">
                 <div>
                    <h4 className="text-purple-700 font-black text-xs uppercase tracking-widest flex items-center gap-2">
                       <Target size={16} /> Kaizen Master
                    </h4>
                    <p className="text-[10px] text-purple-600/70 font-bold mt-1 max-w-xs">
                       User dapat melihat semua absensi & daily report, serta memotong poin pelanggaran anggota tim.
                    </p>
                 </div>
                 <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" className="sr-only peer" checked={formData.isKaizenMaster} onChange={e => setFormData({...formData, isKaizenMaster: e.target.checked})} />
