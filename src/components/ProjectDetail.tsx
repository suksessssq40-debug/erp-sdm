...
export const ProjectDetail = () => {
   const { projects, users, currentUser, settings, updateProject, patchProject } = useAppStore();
   ...
   const notifyTelegram = (title: string, content: string, currentProject: Project, extraInfo?: string) => {
      if (!currentUser) return;
      const collaborators = users.filter(u => currentProject.collaborators.includes(u.id));
      ...
      const msg = `🏢 <b>${escapeHTML(officeLabel)}</b>\n` +
         ...
         `📌 PROYEK: <b>${escapeHTML(currentProject.title)}</b>\n` +
         `${content}\n` +
         (extraInfo ? `\n💡 INFO: ${extraInfo}\n` : '') +
         `\n👤 OLEH: <b>${escapeHTML(currentUser.name)}</b>\n` +
         `👥 TEAM: ${escapeHTML(tagString)}`;

      let targetChatId = currentProject.isManagementOnly ? settings.telegramOwnerChatId : settings.telegramGroupId;
      if (targetChatId) {
   ...
   const postComment = () => {
      const textToNotify = commentText.trim();
      ...
      notifyTelegram(
         "💬 DISKUSI PROYEK (DETAIL VIEW)",
         `<blockquote>"${escapeHTML(textToNotify)}"</blockquote>`,
         project
      );
   };
               ...
               <div className="flex overflow-hidden bg-slate-100 p-1.5 rounded-2xl">
                  {Object.values(KanbanStatus).map(s => (
                     <button
                        ...
                        className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${project.status === s
            ...
            <div className="lg:col-span-4 space-y-8">
               <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
                  ...
                  <div className="pt-6 border-t border-slate-50 space-y-4">
                     <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Dibuat Pada</span>
                        <span className="text-[10px] font-bold text-slate-700">{new Date(project.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                     </div>
                     <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Dibuat Oleh</span>
                        <span className="text-[10px] font-bold text-slate-700">{users.find(u => u.id === project.createdBy)?.name || '-'}</span>
                     </div>
                  </div>
               </div>
               ...
               <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-4">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                     <Users size={14} /> Tim Kolaborator ({project.collaborators.length})
                  </h3>
                  <div className="grid grid-cols-1 gap-2">
                     {project.collaborators.map(uid => {
                        const user = users.find(u => u.id === uid);
                        return (
                           <div key={uid} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-transparent hover:border-blue-100 transition duration-300">
                              <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center text-[10px] font-black uppercase overflow-hidden shadow-sm">
                                 {user?.avatarUrl ? (
                              ...
                              <div>
                                 <p className="text-xs font-black text-slate-700 leading-none mb-1">{user?.name}</p>
                                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{user?.role} • {user?.jobTitle || 'Staff'}</p>
                              </div>
                           </div>
                        );
                     })}
               ...
               <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-4">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center py-6">
                     <p className="text-[10px] font-bold text-slate-400">Belum ada lampiran file.</p>
                     <button className="mt-2 text-[10px] font-black text-blue-600 hover:underline">TAMBAH FILE</button>
                  </div>
               </div>
            </div>

            {/* Right Column: Tasks & Comments */}
                  ...
                  <div className="space-y-4">
                     {project.tasks.map(task => (
                        <div key={task.id}
                           ...
                           className={`p-6 rounded-2xl border-2 cursor-pointer transition-all group hover:shadow-lg ${task.isCompleted ? 'bg-emerald-50 border-emerald-100 opacity-60' : 'bg-white border-slate-100 hover:border-blue-500'}`}
                        >
                           <div className="flex items-start gap-4">
                              <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center mt-0.5 transition ${task.isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 group-hover:border-blue-500'}`}>
