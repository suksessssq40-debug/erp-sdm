...
import { User as UserIcon, Camera, Save, Lock, Mail, AtSign, Briefcase, FileText } from 'lucide-react';
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white/50 flex flex-col items-center text-center space-y-6 relative overflow-hidden group">
                    ...
                    <div className="w-full pt-6 border-t border-slate-50">
                        <div className="flex items-center justify-center gap-2 text-slate-400">
                            <Briefcase size={14} />
                            <span className="text-[10px] font-black uppercase tracking-widest">{formData.jobTitle || 'Belum ada jabatan'}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="col-span-2">
                <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white/50 space-y-8">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Job Title / Jabatan</label>
                            <div className="relative group">
                                <Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-500 transition" size={18} />
                                <input 
                                    type="text" 
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Bio / Tentang Saya</label>
                        <div className="relative group">
                            <FileText className="absolute left-4 top-4 text-slate-300 group-focus-within:text-blue-500 transition" size={18} />
                            <textarea
                                value={formData.bio}
                                onChange={e => setFormData({...formData, bio: e.target.value})}
                                className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl font-bold text-sm text-slate-700 outline-none focus:bg-white focus:border-blue-500 transition shadow-sm h-32 resize-none"
                                placeholder="Ceritakan sedikit tentang peran Anda..."
                            />
                        </div>
                    </div>
