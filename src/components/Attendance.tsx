...
const AttendanceModule: React.FC<AttendanceProps> = ({
  ...
  // Calendar Today (Used for UI display)
  const todayStr = serverTime.toDateString();

  // Effective Today (Matches Server's Shift Logic: < 6 AM is Yesterday)
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <>
              ...
              <div className="space-y-4 w-full">
                ...
                <div className="flex justify-center gap-2">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${strategy === 'FIXED' ? 'bg-blue-50 border-blue-200 text-blue-600' :
      ...
      <div className="space-y-6">
        <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100">
          ...
          <div className="space-y-4">
            {attendanceLog.filter(a => a.userId === currentUser.id && a.date === todayISO).length === 0 ? (
              <EmptyState
                icon={CalendarX2}
              ...
              attendanceLog.filter(a => a.userId === currentUser.id && a.date === todayISO).reverse().map(a => (
