...
export async function POST(request: Request) {
  try {
    ...
    const { id, name, username, telegramId, telegramUsername, role, password, isFreelance, isKaizenMaster } = body;
    ...
    try {
        ...
        return NextResponse.json({ id, name, username, telegramId, telegramUsername, role, isFreelance, isKaizenMaster: !!isKaizenMaster, tenantId }, { status: 201 });
    } catch (e: any) {
        if (e.code === 'P2002') { 
